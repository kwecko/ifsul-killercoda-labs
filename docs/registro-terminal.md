...
O arquivo em `/root/registros` é a transcrição bruta do terminal. Na emissão, `normalizar-registro.pl` (Perl do Ubuntu) cria uma cópia legível: remove cores, títulos e modos ANSI e reconstrói correções comuns de linha, retornos de carro e backspaces, preservando acentos. O bruto permanece intacto. A conversão não é um emulador completo: editores de tela inteira e caracteres de largura especial podem não manter o layout original. Leia como arquivo de texto, sem executar seu conteúdo. Permissões locais: registros e snapshots são privados para root (modo 600 ou diretório privado).
...
A limpeza vale para novas emissões. Arquivos já emitidos ou enviados não são modificados. Não é necessário atualizar o Apps Script: o protocolo e o TXT continuam iguais, e o hash de transporte considera os bytes do LOG legível.
