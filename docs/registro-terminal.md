...
`/root/comprovantes/usuarios-grupos_<matricula>_<sessao>_<hash-do-comprovante>_historico.log`

O hash no nome é o valor hexadecimal do campo CODIGO do comprovante, sem `SHA256:`. O TXT v1 não muda. O histórico não entra no hash interno do TXT; a associação é feita pelo nome do arquivo. O pacote de envio e sua confirmação também usam o SHA-256 dos bytes do próprio histórico para conferir o transporte.
